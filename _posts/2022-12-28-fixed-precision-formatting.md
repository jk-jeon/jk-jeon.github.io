...
Actually, there is yet another reason for me to think about this problem: it is interesting enough to got my attention😃 This post is about my attempt on solving this problem so far.

I want to mention before getting into the main point that this work is a continuation from my previous work on [Dragonbox](https://github.com/jk-jeon/dragonbox), which is an algorithm for spitting out the shortest roundtrip output. As I pointed out, the possibility of excessive precision, which is quite a niche edge case as I claimed above, makes this work much more difficult than Dragonbox. As often in many engineering problems, it is the niche edge case which makes the thing 100x more complicated :)
