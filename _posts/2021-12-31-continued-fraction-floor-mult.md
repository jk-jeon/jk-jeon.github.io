...
{% raw %}
  $\left(\frac{p_{2i}}{q_{2i}}\right)_{i}$
{% endraw %}
of even convergents strictly increases to $x$ from below, while the sequence 
{% raw %}
  $\left(\frac{p_{2i+1}}{q_{2i+1}}\right)_{i}$
{% endraw %} of odd convergents strictly decreases to $x$ from above. In other words, the convergence of convergents happens in a "zig-zag" manner, alternating between below and above of $x$.
