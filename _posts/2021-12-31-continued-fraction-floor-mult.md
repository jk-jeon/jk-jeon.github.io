...
Here is an example. Consider $x=\log_{10}2$. According to [Wolfram Alpha](https://www.wolframalpha.com/), it's hexadecimal expansion is `0x0.4d104d427de7fbcc47c4acd605be48...`. If we choose, for example, $k=20$, then $m=$`0x4d104`$=315652$. Hence, we try to compute $\lfloor nx \rfloor$ by computing
$$
...
```
  (315652 * n) >> 20
```
With a manual verification, it can be seen that this computation is valid for all $|n|\leq 1650$. However, when $n=1651$, the above formula gives $496$ while the correct value is $497$, and when $n=-1651$, the above gives $-497$ while the correct value is $-498$. Obviously, one would expect that the range of valid inputs will increase if we use a larger value for $k$ (thus using more digits of $x$). That is certainly true in general, but it will at the same time make the computation more prone to overflow.
...
Hence, if for example we choose $k=30$, then $m=323228496$, so we must have $|n|\leq \lfloor (2^{31}-1)/m \rfloor = 6$ in order to ensure $nm$ to fit in 32-bits.
...
As a result, these competing requirements will admit a sweet spot that gives the maximum range of $n$. In this specific case, the sweet spot turns out to be $k=21$ and $m=631305$, which allows $n$ up to $|n|\leq 2620$. 
...
$$
  x = a_{0} + \frac{b_{1}}{a_{1} + \frac{b_{2}}{a_{2} + \frac{b_{3}}{a_{3} + \cdots}}},
$$
...
With these conventions, there is a fairly simple algorithm for obtaining a continued fraction expansion of $x$. First, let $a_{0}\coloneqq\lfloor x \rfloor$. Then $x-a_{0}$ is always in the interval $[0,1)$. If it is $0$ that means $x$ is an integer so stop there. Otherwise, compute the reciprocal $x_{1}\coloneqq \frac{1}{x-a_{0}}\in(1,\infty)$. Then let $a_{1}\coloneqq\lfloor x_{1} \rfloor$, then again $x_{1}-a_{1}$ lies in $[0,1)$. If it is zero, stop, and if not, compute the reciprocal $x_{2}\coloneqq\frac{1}{x_{1}-a_{1}}$ and continue.
...
$$
  \frac{415}{93} = 4 + \frac{1}{2+\frac{1}{6+\frac{1}{6+\frac{1}{1}}}}
$$
...
Also, according to [Wolfram Alpha](https://www.wolframalpha.com/) (you can just type `log10(2)`), the continued fraction expansion of $\log_{10}2$ is
$$
...
$$
  \left|x - \frac{p}{q}\right| \leq \left|x - \frac{a}{b}\right|.
$$
...
$$
for all $|n|\leq n_{\max}$? First, note that this equality can be equivalently written as the inequality
$$
...
$$
  \frac{\lceil |n|x \rceil - 1}{|n|}
  = \frac{\lfloor nx \rfloor + 1}{n}
  < \frac{m}{2^{k}}
  \leq \frac{\lfloor nx \rfloor}{n}
  = \frac{\lceil |n|x \rceil}{|n|}.
$$
...
Putting two cases together, we get the following conclusion: if $x$ is irrational or rational with denominator strictly greater than $n_{\max}$, then $(*)$ holds for all $|n|\leq n_{\max}$ if and only if
$$\tag{$\square$}
...
$$
for all $|n|\leq 1165$. In fact, even with $\frac{p_{*}}{q_{*}}=\frac{351}{1166}$ we still have $(\square)$, so the above should hold until the next moment when $\frac{p_{*}}{q_{*}}$ changes, which is when $n_{\max} = 196 + 3\cdot 485=1651$. If $n_{\max}=1651$, then
$$
...
$$
  \lfloor nx \rfloor = \left\lfloor \frac{78913\cdot n}{2^{18}} \right\rfloor
$$
holds precisely up to $|n|\leq 1650$ and the first counterexample is $n=\pm 1651$.
...
$$
must hold at least up to $|n|\leq 2135$. When $n_{\max}=2136$, $\frac{p_{*}}{q_{*}}$ changes into $\frac{643}{2136}$, but $(\square)$ is still true with the same choice of $k$ and $m$, so the above formula must be valid at least for $|n|\leq 2620$; here $2621$ is the moment when $\frac{p^{*}}{q^{*}}$ changes from $\frac{146}{485}$ into $\frac{789}{2621}$. If $n_{\max}=2621$ so $\frac{p^{*}}{q^{*}}$ changes into $\frac{789}{2621}$, then the right-hand side of the inequality $(\square)$ is now violated, so $|n|\leq 2620$ is indeed the optimal range and $n=\pm 2621$ is the first counterexample.
...
$$
  ...
  \leq \frac{b}{q}\left|x - \frac{a}{b}\right|,
$$
so with $b\leq q$ the right-hand side is at most $\left|x-\frac{a}{b}\right|$, so this implies that $\frac{p}{q}$ is a better approximation than $\frac{a}{b}$.
