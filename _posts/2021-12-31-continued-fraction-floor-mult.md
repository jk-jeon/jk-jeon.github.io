...
It can be shown that the sequence $\left(\frac{p_{2i}}{q_{2i}}\right)_{i}$ of even convergents strictly increases to $x$ from below, while the sequence $\left(\frac{p}{q}\right)$ of odd convergents strictly decreases to $x$ from above. In other words, the convergence of convergents happens in a "zig-zag" manner, alternating between below and above of $x$.
